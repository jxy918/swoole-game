...
class JokerPoker {
     ...
     */         
    public static $is_double_rate = array(
        1=>5000, 
        2=>1000,
        3=>4000
    );
     ...
     * 是否翻倍提示语
     */         
    public static $is_double_msg = array(
        1=>'不翻倍回收奖励',
        2=>'再来一次,不回收奖励',
        3=>'奖励翻倍'    
    );
     ...
     */         
	public static function getFiveCard($arr = array()) {
     ...
     */         
    public static function getModValue($arr) {
        $flag = $type = 0;
        $mod = array();
        foreach($arr as $k=>$v) {
            $mod[] = $v % 16;       
    ...
    public static function checkCardType($arr) {
        ...
        $type = self::getModValue($arr_card);
        if(self::isWuZhang($type)) {
            return 9;   //五条
        } elseif(self::isSiZhang($type)) {
            return 7;   //四条
        } elseif(self::isHulu($type)) {
            return 6;   //葫芦，三张两对
        } elseif(self::isSanZhang($type)) {
            return 3;   //三张
        } elseif(self::isLiangDui($type)) {
            return 2;  //两对  
        } else {
            $back = 0;
    ...
    public static function highLight($arr, $type) {
         ...
         if($type > 3) {
            ...
            //对A后者对K
            foreach($card as $k=>$v) {
                if(in_array($v, array('A','K')) || $k == self::$laizi_value) {
                    $card_val[$k] = $v;
                }  
            }                                              
	 ...
	 * 是否翻倍， 玩家翻倍处理
	 */         
	public static function getIsDoubleCard($m_card = 2, $pos = 2, $arr = array()) {
        ...
        //根据key组牌
        $card_val = array($key, $key+16, $key+32, $key+48);
        //去除相同的值
        $card_val = array_diff($card_val, $card_list);   
        $card_key = array_rand($card_val, 1);    
        $card_list[$pos] = $card_val[$card_key];        
        return array('result'=>$rate, 'msg'=>self::$is_double_msg[$rate], 'm_card'=>self::$card_value_list[$m_card],'pos_card'=>self::$card_value_list[$card_list[$pos]], 'pos'=>$pos, 'card'=>$card_list, 'show'=>self::showCard($card_list));        
	}
     ...
     * @return int                             
     */         
    private static function _getRate($arr = array()) {
        $key = 0;
        //首先生成一个1W内的数
        $rid = rand(1, 10000);
        //概率值（按设置累加）
    ...
    public static function getCardType($arr) {
        ...
        return array('card'=>$arr, 'type'=>$type, 'typenote'=>self::$card_type[$type], 'rate'=>self::$card_rate[$type], 'highlight'=>$highlight);   
    }
	
    /**
     * 设置翻倍的概率
     */
    public static function setRate($rate = array()) {
    	if(empty($rate)) {
