...
# 构建swoole环境，在这里安装了php,swoole,composer,安装扩展：phpredis,msgpack,inotify,igbinary,ds,zip opcache bcmath pdo_mysql
RUN apt-get update && apt-get install -y \
	...
	&& phpize && ./configure --enable-async-redis --enable-openssl \
	&& make && make install \
	#源码编译安装inotify
